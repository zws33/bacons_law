...
- **Data:** `Move` = `ActorMove | MovieMove`; `GameState` = `InProgress | GameOver`.
- **Behavior:** `play_move(state, move) -> GameState` and `forfeit(state) -> GameOver`.

Everything else (rules R1–R10) is logic _inside_ `play_move` and `forfeit`.
...
Move = ActorMove | MovieMove  # type alias — the "sealed" set, by convention
```

| Kotlin                                          | Python                                                                                                                          |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `sealed class Move`                             | `Move = ActorMove  MovieMove` (type alias over `@dataclass` types)                                                                      |
| `data class ActorMove(...) : Move()`                | `@dataclass(frozen=True) class ActorMove`                                                                                           |
| `when (m) { is Actor -> ...; is Movie -> ... }` | `if isinstance(m, ActorMove): ... elif isinstance(m, MovieMove): ...`                                                                   |
...
`mypy` does **type narrowing** the same way the Kotlin smart-cast does: inside `if isinstance(previous, MovieMove):`, mypy knows `previous.cast_ids` exists. That gives us most of the safety of a sealed `when` without the language feature.
...
**Naming.** This implementation names the members `ActorMove` / `MovieMove`, not the bare `Actor` / `Movie`. The bare nouns are reserved for the Phase 2/3 wire DTOs and any other domain `Actor` / `Movie` types, so the engine's move variants carry the `…Move` suffix to avoid import collisions when both live in the same namespace. The union alias stays `Move`; downstream code discriminates on the concrete classes, so the suffix has no effect on the rules.
...
Phase 1 scope says _pure, dependency-free logic_. Pydantic is a dependency and a serialization/validation concern; the engine has neither. We use `@dataclass(frozen=True)` from the stdlib. Pydantic models for the wire format (REST DTOs, WS messages) arrive in Phase 2/3 at the API boundary, mapping **to** these engine types — the engine never imports them.
...
The old Kotlin refactor doc used `playMove(move, state)`. `GAME_SPEC_V2` — the authoritative spec for _this_ branch — uses `play_move(state, move)`. We follow the spec. (Flagging because it's a silent contract difference from the Kotlin docs.)
...
Caveat to be aware of: a `frozen=True` dataclass auto-generates `__hash__`, and hashing a `MovieMove` would raise `TypeError` because a `set` is unhashable. **The engine never hashes a `Move`** (repeat detection compares `.id` and type, it doesn't put Moves in a set/dict), so this is latent, not a live bug. If a future phase needs `Move` instances as dict keys or set members, switch `cast_ids` to `frozenset[int]` at that point — `x in cast_ids` works identically for both. Not doing it now keeps the field type identical to the spec.

### D6 — Split `models.py` (data) from `engine.py` (logic)

Mirrors the spec's own "Data Model" vs "Rules" sections and keeps each file small. `__init__.py` re-exports both so callers write `from app.engine import play_move, ActorMove`.
...
├── models.py       # ActorMove, MovieMove, Move, InProgress, GameOver, GameState  (NEW)
...
- `ActorMove`/`MovieMove` field order and optionality match the spec's Data Model section exactly. Display/metadata fields default to `None` so a minimal `ActorMove(id=1, display_text="Tom Hanks")` is valid (as the fixtures use).
...
Lets the API/WS layers (and tests) import from the package root: `from app.engine import play_move, InProgress, ActorMove`. `__all__` declares the public surface and keeps `ruff` from flagging the re-exports as unused imports.
