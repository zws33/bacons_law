...
Sizing is reassuring. A _curated_ catalog of ~50–100k popular movies (rather than the full 1M+ title universe) yields roughly 1.5–2.5M edges, which is a few hundred MB of adjacency data. That fits in RAM on a single modest node, and trivially in Redis. Crucially, **the graph is a fixed cost that does not grow with user count.** It's built offline by a versioned batch ETL job and loaded read-only at boot — never touched in the request path.
...
| Hide validation behind a **`ConnectionChecker` interface**                                                                                                                             | Lets you swap the backing store (in-process map → Redis → something else) without touching game logic.                                       |
| Keep authoritative game state **serializable**                                                                                                                                         | A single in-memory node can later become Redis-backed multi-node without a rewrite.                                                          |
...
| **Node / TypeScript**                     | One language full-stack: shared types between the typeahead client and server validation; discriminated unions model the entities cleanly.                                                                                                    | Single-threaded per process (scale via N processes + sticky sessions / Redis pub-sub); broadcast fan-out pressures the event loop. |
| **C# / .NET**                             | Underrated: SignalR makes WebSocket management, reconnection, and a Redis backplane nearly turnkey.                                                                                                                                           | Smaller indie/hobby mindshare — cultural more than technical.                                                                      |
