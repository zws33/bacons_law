# Decision Log — Kotlin/Android implementation (archived)

> **Archived.** These ADRs (001–007) record decisions for the original Kotlin/Compose
> Android implementation, which lives on the `main` branch. They are preserved here for
> historical context. On the `fullstack-py-ts-rewrite` branch they are **superseded** —
> see [`docs/DECISIONS.md`](../DECISIONS.md) for the rewrite-era decision log (ADR 008
> onward) and [`docs/PYTHON_TS_REWRITE_PLAN.md`](../PYTHON_TS_REWRITE_PLAN.md) for the
> current project's direction. Where a rewrite-era ADR supersedes one of these, it says so.
...
> **Superseded on the rewrite branch by [`docs/DECISIONS.md` ADR 009](../DECISIONS.md):
> multi-device play is a core requirement; pass-the-phone was the local-only MVP.**
...
> **Note:** The rewrite branch hosts on **Fly.io, not Cloud Run** — persistent WebSocket
> connections are incompatible with scale-to-zero. See
> [`docs/PYTHON_TS_REWRITE_PLAN.md`](../PYTHON_TS_REWRITE_PLAN.md) ("Hosting is Fly.io").
> The credentials-never-in-client-binaries invariant carries over unchanged.
...
> **Note:** The rewrite branch realizes this server-authoritative inversion from the start
> (the backend owns `GameState`; clients are thin). See
> [`docs/DECISIONS.md` ADR 009](../DECISIONS.md).
