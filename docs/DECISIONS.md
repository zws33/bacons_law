...
Lightweight ADR-style record of key technical and product decisions for the **Python/TypeScript
multi-device rewrite** (`fullstack-py-ts-rewrite` branch).

> ADRs 001–007 cover the original Kotlin/Android implementation (on `main`) and are archived at
> [`docs/kotlin/DECISIONS.md`](kotlin/DECISIONS.md). Numbering continues from there: this log starts
> at 008. The rewrite's broader direction lives in
> [`docs/PYTHON_TS_REWRITE_PLAN.md`](PYTHON_TS_REWRITE_PLAN.md).
...
**Supersedes the product framing of:** [Kotlin ADR 001](kotlin/DECISIONS.md#001-mvp-is-pass-the-phone-two-player-not-solo-chain-building) (pass-the-phone is no longer the end state — it was the original single-device MVP).
